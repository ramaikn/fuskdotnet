...
| **Control Flow Flattening** | Restructures method logic into switch-dispatch loops with randomized state values and physical layout. Inserts unreachable code blocks (bogus blocks) gated by opaque predicates (expressions that always evaluate to false at runtime) to create fake edges in the Control Flow Graph (CFG), confusing static analyzers and disassemblers. |
...
| **Call Indirection (Call Proxying)** | Wraps non-virtual method calls into static forwarder stubs. Generates up to 6 interchangeable stubs per target and distributes call sites randomly across them to disrupt call-graph mapping, preventing easy deobfuscator call-site folding. |
