...
- **Execution-Bound Key Protection:** String and constant keys are tied to values that only materialize when the program is genuinely executed on a real runtime. A tool that folds the binary statically, emulates it with a generic IL interpreter, or invokes a decryption routine in isolation recovers the wrong key — pushing automated static deobfuscation into full dynamic execution of the whole application.
...
| **Runtime Image Key (RIK)** | Folds an additional always-zero term into every string/constant key mask, reconstructed at runtime through value-producing framework calls (`Math.Abs`, `BitConverter.DoubleToInt64Bits`). The term is zero only when a real runtime executes the chain: the shortcuts a generic IL folder/emulator uses — reading an unset field as its default, stubbing an unmodeled call to zero, or passing an argument through — each yield the wrong value and a corrupted key. Independent of the module seed, so it also resists a symbolic solver, at the cost of a single field read per method. |
| **Execution Path Key (EPK)** | Ties the key mask of methods that run only after startup to a value the entry point sets as its first action. A decryptor-invoker that calls a decryption routine cold — without running the program, the technique `de4dot`'s dynamic string mode uses — reads the default value and computes the wrong key, forcing it to execute the entire application instead of a single function. A conservative reachability analysis restricts the binding to methods provably reached only after startup, and it is automatically inert on library assemblies. |
| **Poly-Decoy Injection** | Injects decoy validation-shaped methods reached from a random subset of real methods behind a fixed-per-build guard that never runs at runtime. The decoys are dead but a static tool cannot prove them dead, so hunting the real "decrypt-then-compare" logic collides with many indistinguishable candidates. The guard is a field read plus a not-taken branch; binary growth is bounded by a configurable ratio. |
