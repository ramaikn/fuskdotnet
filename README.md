...
| **Control Flow Flattening** | Restructures method logic into switch-dispatch loops with randomized state values and physical layout. Adds unreachable bogus dispatcher arms (so arm count no longer maps to real blocks) and opaque-predicate guards that vary sense (`brtrue`/`brfalse`) and mix a live method argument into the always-zero term, creating fake CFG edges a dynamic evaluator cannot fold from the static seeds alone. |
